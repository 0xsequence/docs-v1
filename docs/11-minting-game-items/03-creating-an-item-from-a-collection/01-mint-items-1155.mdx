...
In this guide, we’re going to walk through the process of creating your own in-game assets through minting from an ERC-1155 contract. These assets commonly used for items and acheivements, but could be used for just about any game object.

#### An ERC-1155?

You might be wondering, why use an “ERC-1155”? Or maybe it means nothing to you at all, and you just want to create an awesome game with some of its data stored on-chain.
...
Before you get started creating (minting) items, you will need to first deploy the smart contract you wish to mint from. If you have not done so already, go back and do [how to load an item collection through deploying a smart contract](https://docs.sequence.xyz/minting-game-items/deploy-an-item-collection).
...
Since we're here to mint item today, select the `mint` method.
