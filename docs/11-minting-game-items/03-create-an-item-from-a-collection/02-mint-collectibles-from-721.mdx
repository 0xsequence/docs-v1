---
slug: /mint-collectibles-from-721
---
...
For more on creating in-game items from collections, [check our guide on using ERC-1155](https://docs.sequence.xyz/mint-items-from-1155) to do that.
...
Before you get started creating (minting) collectibles, you will need to first deploy the smart contract you wish to mint from. If you have not done so already, go back and do [how to load an item collection through deploying a smart contract](https://docs.sequence.xyz/deploy-an-item-collection).
...
Once in the project dashboard, you should see the ERC-721 contract that you have deployed - select it.
...
Within that subset of options, select the `Write Contract` section.



![Sequence builder write contract](/img/builder/builder_select_write_contract_mint_collectibles.png)

This will drop down several methods that you can call to the smart contract by sending a transaction to it.
...
Once you hit `write`, a modal with your wallet will pop-up. In order to create (mint) these items, you will need to send a transaction.
...
In my example, you can see the 5 collectibles that we created.
