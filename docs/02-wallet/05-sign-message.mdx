...
by making a simple call remote API call.
...
The above example shows the raw HTTPS request/response to the Sequence API, but you may also utilize [`sequence.js`](https://github.com/0xsequence/sequence.js) or
[`go-sequence`](https://github.com/0xsequence/go-sequence) SDKs which provide a full Sequence client.

```ts
// Using the 0xsequence npm package from either a Webapp (browser) or Node.js:
