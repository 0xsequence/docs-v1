...
Our marketplace provides a series of RPC endpoints that you can use to interact with the Sequence Orderbook - a protocol for custom orders - and user activities within the stack.

## GetTopOrder
Query the order book for the top order based on various filters.

Fetch the top orders from the order book, optionally excluding those created by a specific user.

* Request: POST /rpc/Marketplace/GetTopOrders
* Content-Type: application/json
* Body (in JSON):
    * `orderbookContractAddress` (string) -- the address of the order book contract
...
Query the order book for orders based on various filters.
