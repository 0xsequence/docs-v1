...
The Sequence Relayer can be implemented on a severless [Cloudflare](https://cloudflare.com/) worker so a game or app user interaction is seamless without a confirmation signature or gas payment. You'll also benefit from not having to be worried about transaction speed, throughput and re-orgs.
...
Follow the prompts, then once logged in initialize the project in the project directory and follow the prompts to deploy yout typescript application.
...
Build your smart contract with the necessary features, for example, here is the recommended contract that adds a few features like: only being able to mint from the relayer, the ability to turn on and off the mint, and and owner associated with the contract address.
