---
slug: /indexer
---

# Indexer

The Sequence Indexer is a modular web3 service that makes it easy to query blockchain token
and NFT data from Ethereum-compatible networks. The Indexer **automatically indexes every
ERC20, ERC721 and ERC1155 token** from Ethereum-compatible chains.

In fact, the Sequence Wallet uses the Indexer behind the scenes so it can seamlessly
render all token information in any wallet. But of course, the Indexer is a modular piece
of infrastructure, and you may use it directly from your dapp, game, or even from
a server.

**Features:**

- Super-fast API to query all token balances, history, metadata and NFTs with multi-chain support
- Real-time indexing of ERC20, ERC721 and ERC1155 transactions across EVM-compatible chains
- Automatically detects all tokens on the chain, without the need for a contract registry
- Resilient to node failures and chain re-organizations
- Easily listen for specific events and transactions on-chain accurately with a simple API
- Built-in token / nft metadata support to easily render tokens in your apps / games
- High uptime and availability

## Supported Networks & Endpoints

**Status:** https://status.sequence.info

import IndexerTable from '/src/components/IndexerTable'
const serviceList = [
  {
    network: 'Ethereum',
    logo: '/img/networks/1.png',
    endpoint: 'https://mainnet-indexer-v2.sequence.app',
  },
  {
    network: 'Polygon',
    logo: '/img/networks/137.png',
    endpoint: 'https://polygon-indexer-v2.sequence.app',
  },
  {
    network: 'Arbitrum One',
    logo: '/img/networks/42161.png',
    endpoint: 'https://arbitrum-indexer-v2.sequence.app',
  },
  {
    network: 'Arbitrum Nova',
    logo: '/img/networks/42170.png',
    endpoint: 'https://arbitrum-nova-indexer-v2.sequence.app',
  },
  {
    network: 'Optimism',
    logo: '/img/networks/10.png',
    endpoint: 'https://optimism-indexer-v2.sequence.app',
  },
  {
    network: 'BSC',
    logo: '/img/networks/56.png',
    endpoint: 'https://bsc-indexer-v2.sequence.app',
  },
  {
    network: 'Avalanche',
    logo: '/img/networks/43114.png',
    endpoint: 'https://avalanche-indexer-v2.sequence.app',
  },
  {
    network: 'Gnosis',
    logo: '/img/networks/64.png',
    endpoint: 'https://gnosis-indexer-v2.sequence.app',
  },
]
const testServiceList = [
  {
    network: 'Goerli',
    logo: '/img/networks/1.png',
    endpoint: 'https://goerli-indexer-v2.sequence.app',
  },
  {
    network: 'Mumbai',
    logo: '/img/networks/137.png',
    endpoint: 'https://mumbai-indexer-v2.sequence.app',
  },
  {
    network: 'BSC Testnet',
    logo: '/img/networks/56.png',
    endpoint: 'https://bsc-testnet-indexer-v2.sequence.app',
  },
]

**Mainnet networks:**

<IndexerTable items={serviceList} />

<br />

**Test networks:**

<IndexerTable items={testServiceList} />

## Getting Started

Here are a few example queries you can make to a blockchain from your dapp, game, or wallet:

- [Fetch all tokens & NFTS in any wallet including all metadata](/indexer/guides/fetch-tokens)
- [Fetch the transaction history for any wallet address](/indexer/guides/transaction-history)
- [Fetch all unique tokens in a particular ERC20/721/1155 contract, including total supplies](/indexer/guides/unique-tokens)
- [What is the total token supply of an ERC20 token? What is the total token supply of
  all the ERC1155 tokens in a particular contract?](/indexer/guides/unique-tokens)
- [Fetch the transaction history for any token contract address](/indexer/guides/transation-history-token-contract)

## Pricing & Usage

The Sequence Indexer is available for free with moderate request limits, but if your project
requires higher limits, please contact the [Sequence Pro](https://pro.sequence.xyz) team.
