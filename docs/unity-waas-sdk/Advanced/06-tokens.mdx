...
```
TransactionReceipt receipt = await erc20.Mint(toAddress, 1).SendTransactionMethodAndWaitForReceipt(wallet, client);
```
As a wrapper of `Contract`, you also have the option to not create the `EthTransaction` and send later on.
```
CallContractFunction transactionCreator = erc20.Transfer(toAddress, 1);
