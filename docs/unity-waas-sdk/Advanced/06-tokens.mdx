...
```
You may also provide your own ABI should you need to rewrite our default.
...
```
TransactionReceipt receipt = await erc20.Mint(toAddress, DecimalNormalizer.Normalize(1)).SendTransactionMethodAndWaitForReceipt(wallet, client);
```
As a wrapper of `Contract`, you also have the option to not create the `EthTransaction` and send later on.
```
CallContractFunctionTransactionCreator transactionCreator = erc20.Transfer(toAddress, DecimalNormalizer.Normalize(1));
