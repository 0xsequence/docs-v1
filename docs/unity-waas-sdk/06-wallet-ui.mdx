...
To add the UI to your scene, you can add the `WalletPanel` prefab found under `Assets > SequenceExamples > Prefabs`. It is opened using the Open method; you must provide an `Address` as an argument - this is the address for the wallet and will be used to read from the blockchain using the Indexer.

You'll notice that the UI is still a work in progress and is using mocks in some places (fetching transaction history and prices). These will be replaced in coming updates of the SDK, but please feel free to replace them on your own (and for bonus points, submit a PR!).
...
To help you with the process, we've added a basic `ColorSchemeManager` script which you can experiment with in our Demo scene (found at `Assets > SequenceExamples > Scenes > Demo.unity`).
