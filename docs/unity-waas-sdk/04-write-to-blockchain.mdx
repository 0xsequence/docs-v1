...
_wallet.SendTransaction(new SendTransactionArgs(
    _address, Chain.Polygon, new SequenceSDK.WaaS.Transaction[]
    {
        new RawTransaction(myToken.Mint(toAddress, amount)),
    }));
```
...
with this reference, you'll have access to all of the methods implemented by the ERC721 class. Any method that returns a `CallContractFunction`, e.g. `Burn`, can be used when creating a RawTransaction with WaaS. For example:
...
_wallet.SendTransaction(new SendTransactionArgs(
    ...
    new SequenceSDK.WaaS.Transaction[]
    {
        ...
        new SendERC20(
            ...
            ToAddress,
            AmountAsString),
        new RawTransaction(new ERC20(erc20TokenAddress).Burn(amount)),
        new SendERC721(
            erc721TokenAddress,
            ToAddress,
            TokenIdAsString),
