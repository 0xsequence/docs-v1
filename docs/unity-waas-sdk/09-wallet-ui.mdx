...
To add the UI to your scene, you can add the `WalletPanel` prefab found under `Assets > SequenceExamples > Prefabs`. It is opened using the Open method; you must provide an [`IWallet`](https://github.com/0xsequence/sequence-unity/blob/master/Assets/SequenceSDK/WaaS/IWallet.cs) (e.g. a `WaaSWallet`) as an argument.
...
To help you with the process, we've added a basic [`ColorSchemeManager` script](https://github.com/0xsequence/sequence-unity/blob/master/Assets/SequenceExamples/Scripts/UI/ColorSchemeManager.cs) which you can experiment with in our Demo scene (found at `Assets > SequenceExamples > Scenes > Demo.unity`).
