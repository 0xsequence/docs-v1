# Authentication
As a WaaS SDK, authentication is extremely important. Authentication works via a unique combination of AWS KMS encryption and by establishing a session signing wallet through association with an [OIDC idToken](https://auth0.com/docs/secure/tokens/id-tokens/id-token-structure#sample-id-token). For more on how WaaS works, please see: https://docs.sequence.xyz/waas/intro/
...
No additional steps required. Our SDK will automatically append the `UrlScheme` you set in `Resources/SequenceConfig` to your Plist.
