...
DeployContract is essentially a wrapper for a very special `WaaSWallet.SendTransaction` call and therefore, you can expect to still receive the `WaaSWallet.OnSendTransactionComplete` or `WaaSWallet.OnSendTransactionFailed` events in addition.

```
public void OnDeployContractCompleteHandler(SuccessfulDeployContractReturn result) {
    Address newlyDeployedContractAddress = result.DeployedContractAddress;

    // Do something
}

public void OnDeployContractFailedHandler(FailedDeployContractReturn result) {
    // Do something
}
