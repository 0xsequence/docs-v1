# Authentication
As a WaaS SDK, authentication is extremely important. Authentication works via a unique combination of [AWS KMS](https://aws.amazon.com/kms/) encryption and by establishing a session signing wallet through association with an [OIDC idToken](https://auth0.com/docs/secure/tokens/id-tokens/id-token-structure#sample-id-token). For more on how WaaS works, please see: https://docs.sequence.xyz/waas/intro/
