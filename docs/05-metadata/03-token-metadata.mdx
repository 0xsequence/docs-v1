...
When deploying new contracts or metadata updates the refresh token metadata url should be called via a simple HTTPS get reguest by command line or browser for data to indexeable in the Sequence Indexer metadata service.
