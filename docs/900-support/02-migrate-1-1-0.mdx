...
Note: with v1.7.0, projectAccessKey is now passed as first parameter of initWallet and is required.
...
```ts
const projectAccessKey = '<access_key>'
const provider = sequence.initWallet(projectAccessKey, { defaultNetwork: 'mainnet' })
