...
After you connect, you can use `wallet.open` to open the wallet:

```ts
wallet.openWallet();
```

You can also optionally pass a path, and use `openWithOptions` intent to pass ConnectOptions when you open the wallet:
```ts
