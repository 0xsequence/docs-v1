...
can on-boarding with a familiar Web2 experience. Think of it like *Paypal / Stripe but for Web3*. 
...
Most browsers will block popups if they are called outside of user-triggered event handlers like `onclick`, or when it takes too long to process between the user action and the actual window open.
...
This requires users to use the Sequence Chrome Extension or Wallet Connect. This is identical to connecting with MetaMask.

NOTE: to offer the on-demand / no-install Web Wallet to your users, please see section above on connecting with `0xsequence` npm package.
Also note, that you can use Sequence 0xsequence in combination with Metamask and web3.js and ethers.js.

Finally, just FYI, 0xsequence is actually built on ethers.js :)
