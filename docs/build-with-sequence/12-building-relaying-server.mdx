...
By default, Sequence transactions will be executable sequentially. 
:::
...
Also, the Sequence wallet should be sufficiently funded in order to pay the fee needed by the relayer to dispatch your transactions.
...
You can enforce that transactions can be executed in parallel by using a random nonce space, as follow; 
...
You can also send transactions in batches using `await signer.sendTransactionBatch(txns)`. You can read more about batch transactions in [Sending Batched Transactions](/build-with-sequence/send-batch-transactions).
