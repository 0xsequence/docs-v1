...
The connector is installed via the npm package `@0xsequence/wagmi-connector`. If you are using the latest, viem-based, version of wagmi (>= 1.0.0) the latest version of the wagmi-connector package can be installed with the following command:
...
If you are using an older version of Wagmi which is based of ethers (<= 0.12.x), use the following command:
