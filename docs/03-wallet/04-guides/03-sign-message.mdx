...
:::warning[changes to sequence API usage and billing]
Our **Sequence API** services are now managed through **Sequence Builder**. Usage is charged in credits - which are issued monthly based your subscription plan. [Get started with Sequence Builder today](https://sequence.build/landing)!
:::
...
:::caution
Notice that signing non-EIP6492 signatures requires the wallet to be deployed onchain, and thus the user will be prompted
to deploy the wallet onchain before being able to sign the message. If the network has fees (like Ethereum), the user will
also be prompted to pay the transaction fees.
:::
