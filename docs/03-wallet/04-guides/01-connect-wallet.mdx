...
If you provide only a single sign in option, like Google or Email, the wallet will immediately open that login prompt & skip the login provider selection screen.
```ts
const wallet = sequence.getWallet()
...
If you pass only one, it'll open that login screen immediately.
