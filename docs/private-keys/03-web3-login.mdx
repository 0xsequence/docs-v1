...
If this path was chosen, users wallets would only be controlled by session keys stored on their devices and a [backup key](backup_key). This also implies that these wallets are not associated with a Guard Key [**LINK**] either and hence users are fully responsible for ensuring they do not lose access to their keys.

## Wallet Creation
When creating a wallet using the web3 login, users will create a session key that will be stored on their device. In addition, a [backup key](backup_key) will also be generated and users will need to store them securely.
